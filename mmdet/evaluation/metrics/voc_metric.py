...
class VOCMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for idx, data_sample in enumerate(data_samples):
            ...
            gt_ignore_instances = data_sample['ignored_instances']
            ann = dict(
                labels=gt_instances['labels'].detach().cpu().numpy(),
                bboxes=gt_instances['bboxes'].detach().cpu().numpy(),
                bboxes_ignore=gt_ignore_instances['bboxes'].detach().cpu().numpy(),
                labels_ignore=gt_ignore_instances['labels'].detach().cpu().numpy())

            pred = data_sample['pred_instances']
            pred_bboxes = pred['bboxes'].detach().cpu().numpy()
