...
def mmdet_attack_voc_ncc(runner: Runner, ncc_thres, attack_threshold , start_iter, end_iter, path_name):
    ...
    for idx, data_batch in enumerate(runner._val_loop.dataloader):
        ...
        for number_of_steps in range(total_steps):
            ...
            if number_of_steps == total_steps - 1 or flag_zero_pred == True:
                path_save = os.path.split(data['data_samples'][0].img_path)[1]
                save_image(inverse_to_base(gradient_mask),f'voc_{path_name}/{path_save}')
                break
...
def main(ncc_thres, attack_threshold, start_iter, end_iter):
    ...
    if os.path.exists(f"voc_{path_name}") == False:
        os.mkdir(f"voc_{path_name}")
    ...
    # start training
    mmdet_attack_voc_ncc(runner, ncc_thres, attack_threshold, start_iter, end_iter, path_name)
